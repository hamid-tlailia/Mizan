...
export function HadithSearch({ value, isLoading, onChange, onSubmit, onSuggestion }: HadithSearchProps) {
  return (
    <section className="relative mx-auto w-full max-w-4xl" aria-labelledby="search-title">
      <div className="absolute -inset-8 -z-10 rounded-[3rem] bg-stone-500/5 blur-3xl" />
      <div className="research-card overflow-hidden p-1.5 sm:p-2">
        <div className="rounded-[1.2rem] bg-card/75 p-4 sm:p-6">
          <div className="mb-4 flex items-center gap-3 text-right">
            <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-stone-600 text-white shadow-lg shadow-stone-700/15"><BookOpenCheck className="size-5" /></span>
            <div>
          ...
          <textarea
            ...
            className="hadith-input min-h-44 w-full resize-y rounded-2xl border border-border bg-background/80 p-4 text-right text-lg leading-9 text-foreground outline-none transition focus:border-stone-600 focus:ring-4 focus:ring-stone-600/10 disabled:cursor-not-allowed disabled:opacity-60 sm:p-5"
          ...
          <div className="mt-3 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button onClick={onSubmit} disabled={isLoading || value.trim().length < 12} size="lg" className="h-12 rounded-xl bg-stone-700 px-6 text-base font-bold shadow-lg shadow-stone-900/15 hover:bg-stone-800 dark:bg-stone-600 dark:hover:bg-stone-500">
